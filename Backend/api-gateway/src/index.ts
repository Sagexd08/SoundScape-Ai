...
// Service Routing - Protected routes use auth middleware
...
app.use('/api/users', supabaseAuthMiddleware, createProxyMiddleware({
  target: process.env.USER_SERVICE_URL || 'http://user-service:4001',
  changeOrigin: true,
  pathRewrite: { '^/api/users': '' }
}));

app.use('/api/audio', supabaseAuthMiddleware, createProxyMiddleware({
...
app.use('/api/storage', supabaseAuthMiddleware, createProxyMiddleware({
  target: process.env.STORAGE_SERVICE_URL || 'http://storage-service:4003',
  changeOrigin: true,
  pathRewrite: { '^/api/storage': '' }
}));

app.use('/api/recommendations', supabaseAuthMiddleware, createProxyMiddleware({
  target: process.env.RECOMMENDATION_ENGINE_URL || 'http://recommendation-engine:4004',
  changeOrigin: true,
  pathRewrite: { '^/api/recommendations': '' }
}));

app.use('/api/analytics', supabaseAuthMiddleware, createProxyMiddleware({
  target: process.env.ANALYTICS_SERVICE_URL || 'http://analytics-service:4005',
  changeOrigin: true,
  pathRewrite: { '^/api/analytics': '' }
}));

app.use('/api/notifications', supabaseAuthMiddleware, createProxyMiddleware({
